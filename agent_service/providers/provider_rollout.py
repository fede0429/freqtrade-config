...
class ProviderRolloutPolicy:
    ...
    def required_providers_for_pair(self, pair: str) -> List[str]:
        pair = pair.upper()
        overrides = self.pair_overrides().get(pair, {})
        if "required_providers" in overrides:
            return list(overrides["required_providers"])
        return list(self.config.get("required_provider_names", []))

    def rollout_stage_for_pair(self, pair: str) -> str:
        pair = pair.upper()
        overrides = self.pair_overrides().get(pair, {})
        return str(overrides.get("rollout_stage", self.config.get("rollout_stage", "default")))
