...
from agent_service.aggregator.rollout_state_machine import RolloutStateMachine
...
class DecisionAggregator:
    def __init__(
        ...
        self.rollout_state_machine = RolloutStateMachine(rollout_policy or {})
    ...
    def build_pair_decision(self, pair: str, snapshots: List[ProviderSnapshot]) -> Dict[str, Any]:
        ...
        rollout = self.rollout_state_machine.evaluate(
            pair=pair,
        ...
        entry_allowed = entry_allowed and bool(governance.get("approved", False)) and not bool(rollout.get("frozen", False))
        ...
        return {
            "agent_enabled": provider_gate["provider_gate_passed"] and not anomaly["blocking"] and bool(governance.get("approved", False)) and not bool(rollout.get("frozen", False)),
            "pair_enabled": True,
            "governance_gate": "passed" if governance.get("approved", False) else "blocked",
            "governance_gatekeeper": governance,
            "rollout_state_machine": rollout,
            ...
            "entry": {
                ...
                "entry_reason": "governed_rollout_alignment" if entry_allowed else "governance_or_rollout_or_provider_or_anomaly_or_confidence_or_risk_or_cooldown_blocked",
                "entry_min_confidence": entry_min_confidence,
            },
            "stake": {
                "stake_multiplier": stake_multiplier if callback_policy["stake"] and governance.get("approved", False) and not rollout.get("frozen", False) else 1.0,
                "stake_cap_ratio": float(cfg.get("stake_cap_ratio", 0.12)),
                "stake_reason": "weighted_provider_signal" if callback_policy["stake"] and governance.get("approved", False) and not rollout.get("frozen", False) else "stake_disabled_by_governance_or_rollout_or_execution_policy",
            },
            "exit": {
                "exit_signal": False,
                "exit_reason": None,
            ...
            "roi": {
                "target_rr": target_rr if callback_policy["roi"] and governance.get("approved", False) and not rollout.get("frozen", False) else None,
            ...
            "rollout_state": rollout.get("rollout_state", "shadow"),
            "entry_allowed": entry_allowed and callback_policy["entry_confirm"],
            "stake_multiplier": stake_multiplier if callback_policy["stake"] and governance.get("approved", False) and not rollout.get("frozen", False) else 1.0,
            "exit_signal": False,
            "exit_reason": None,
