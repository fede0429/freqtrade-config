...
class AgentBridgeStrategy(BaseStrategy):
    ...
    def _decision_value(self, decision, nested_key, flat_key, default=None):
        ...
        if group and subkey and isinstance(decision.get(group), dict) and subkey in decision[group]:
            return decision[group][subkey]
        return decision.get(flat_key, default)
    def _agent_enabled_for_pair(self, pair):
        d = self._pair_decision(pair)
        return bool(d) and self._pair_allowed(pair) and self._cache_is_fresh() and d.get("governance_gate") == "passed"
    def _trace(self, filename, payload): self.audit.append_event(filename, payload)

    def bot_loop_start(self, current_time, **kwargs):
        self._refresh()
        self._trace("bridge_runtime_trace.jsonl", {"shadow_mode": self.shadow_mode, "enabled_callbacks": self.enabled_callbacks, "cache_fresh": self._cache_is_fresh(), "pair_count": len(self._cache.get("pairs", {}))})
    ...
    def custom_stake_amount(self, pair, current_time, current_rate, proposed_stake, min_stake, max_stake, leverage, entry_tag, side, **kwargs):
        d = self._pair_decision(pair)
        self._trace("stake_decision_trace.jsonl", {"pair": pair, "mode": "shadow" if self.shadow_mode else "live", "proposed_stake": proposed_stake, "decision": d})
        if self.shadow_mode or not self.enabled_callbacks.get("stake", False): return proposed_stake
        if not self._agent_enabled_for_pair(pair): return proposed_stake
        return min(proposed_stake * float(self._decision_value(d, "stake.stake_multiplier", "stake_multiplier", 1.0)), max_stake or proposed_stake)

    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        d = self._pair_decision(pair)
        self._trace("exit_shadow_trace.jsonl", {"pair": pair, "mode": "shadow" if self.shadow_mode else "live", "current_profit": current_profit, "decision": d})
        if self.shadow_mode: return None
        return self._decision_value(d, "exit.exit_reason", "exit_reason", None) if bool(self._decision_value(d, "exit.exit_signal", "exit_signal", False)) else None

    def custom_stoploss(self, pair, trade, current_time, current_rate, current_profit, after_fill, **kwargs):
        d = self._pair_decision(pair)
        self._trace("stoploss_shadow_trace.jsonl", {"pair": pair, "mode": "shadow" if self.shadow_mode else "live", "current_profit": current_profit, "decision": d})
        if self.shadow_mode: return None
        return self._decision_value(d, "stoploss.agent_stoploss", "agent_stoploss", None)

    def custom_roi(self, pair, trade, current_time, trade_duration, entry_tag, side, **kwargs):
        d = self._pair_decision(pair)
        self._trace("roi_shadow_trace.jsonl", {"pair": pair, "mode": "shadow" if self.shadow_mode else "live", "trade_duration": trade_duration, "decision": d})
        if self.shadow_mode: return None
        return self._decision_value(d, "roi.target_rr", "target_rr", None)

    def confirm_trade_entry(self, pair, order_type, amount, rate, time_in_force, current_time, entry_tag, side, **kwargs):
        d = self._pair_decision(pair)
        self._trace("entry_confirm_trace.jsonl", {"pair": pair, "mode": "shadow" if self.shadow_mode else "live", "amount": amount, "rate": rate, "decision": d})
        if self.shadow_mode: return True
        return bool(self._decision_value(d, "entry.entry_allowed", "entry_allowed", True))
